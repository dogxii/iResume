...
import { normalizeResumeData, STANDARD_SECTION_KEYS } from './resumeData'
import {
...
export const computeNextVersion = (
  ...
  const next =
    bump === 'minor' ? incrementMinorVersion(base) : incrementPatchVersion(base)
...
export function normalizeDocumentHistory(value: unknown): DocumentHistory {
  ...
  const snapshots: HistorySnapshot[] = raw.snapshots
    ...
    .filter(
      (item) => typeof item.document === 'object' && item.document !== null
    )
    .map((item) => {
...
const getCustomSectionTitle = (data: ResumeData, section: CustomSection) =>
  data.sectionTitles[section.id]?.trim() || section.title.trim() || '自定义区块'
...
const diffResumeData = (
  ...
  const visibilityChanges = SECTION_KEYS.filter(
    ...
    (key) =>
      `${after.sectionVisibility[key] ? '显示' : '隐藏'}${SECTION_LABELS[key]}`
  )
...
export function formatSnapshotDiffSummary(diff: SnapshotDiff): string {
  ...
  const scopes = Array.from(
    new Set(diff.changes.map((change) => change.scope))
  ).slice(0, 2)
