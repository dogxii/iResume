...
>(function StructuredResumeLayout(
  ...
  const photoSize = Math.round(58 * sectionPreferences.personal.photoSizeRatio)
        ...
        return (
          <div key={item.id} className='mb-[0.78em] last:mb-0'>
            ...
            >
              ...
              {roleInMiddle && (
                <span
                  className={`justify-self-end whitespace-nowrap ${experienceRoleToneClass}`}
                >
                  {role}
                </span>
              )}
            ...
            {roleAtBottom && (
              <div
                className={`mt-[0.18em] px-[0.48em] leading-[1.35] ${experienceRoleToneClass}`}
              >
                {role}
              </div>
            )}
        ...
        const tagNode =
          showTags && tags ? (
            sectionPreferences.projects.tagStyle === 'text' ? (
              ...
              <span className='inline text-neutral-700'>{tags}</span>
            )
          ) : null
        const tagOnTitle =
          sectionPreferences.projects.tagPosition === 'title' ? tagNode : null
        const tagBelow =
          sectionPreferences.projects.tagPosition === 'below' ? tagNode : null
          ...
          <div key={project.id} className='mb-[0.82em] last:mb-0'>
            ...
            >
              ...
              <span className='min-w-0 justify-self-center text-center'>
                ...
                {tagOnTitle && <span className='ml-[0.7em]'>{tagOnTitle}</span>}
              </span>
              {hasRightRole && (
                <span
                  className={`justify-self-end whitespace-nowrap ${projectRoleToneClass}`}
                >
                  {role}
                </span>
              )}
            ...
            {belowMeta && (
              <div className='mt-[0.18em] flex min-w-0 flex-wrap items-center gap-x-[0.85em] gap-y-[0.18em] px-[0.48em] leading-[1.35] text-neutral-700'>
                {roleAtBottom && (
                  <span className={projectRoleToneClass}>{role}</span>
                )}
                {linksBelow}
                {tagBelow}
              </div>
        ...
        {visibleItems.map((item) => (
          <div key={item.id} className='mb-[0.78em] last:mb-0'>
            <div className='print-item-header grid grid-cols-[max-content_minmax(0,2fr)_minmax(0,1.35fr)] gap-x-[1.2em] px-[0.48em] leading-[1.42] text-neutral-900'>
              ...
              <span className='min-w-0 text-right'>{item.subtitle}</span>
            </div>
            <StructuredMarkdownBlocks
              text={item.details}
              className='mt-[0.2em]'
