...
import { parseInline, renderMarkdownBlocks } from '../utils/markdown'
...
const BannerLink = ({
  ...
  const content = (
    <>
      ...
      <span className={bannerLinkAddressClasses[style]}>{text}</span>
    </>
  )
...
const ResumePreview = forwardRef<HTMLDivElement, ResumePreviewProps>(
  function ResumePreview(
    ...
    const shouldIgnoreSectionClick = (event: React.MouseEvent<HTMLElement>) => {
      ...
      switch (template.sectionHeaderStyle) {
        ...
        case 'double-line':
          return (
            <div className='mb-3'>
              <h2
                className={`resume-section-title text-base font-bold ${c.heading} pb-1`}
              >
                {title}
              </h2>
        ...
        default:
          return (
            <h2
              className={`resume-section-title text-lg font-bold ${c.heading} mb-2`}
            >
              {title}
            </h2>
          )
        ...
        >
          {hasGithub && (
            <BannerLink
              ...
              icon={
                (styledLinks || template.showLinkIcons) && <Github size={14} />
              }
          ...
          {hasWebsite && (
            <BannerLink
              ...
              icon={
                (styledLinks || template.showLinkIcons) && (
                  <UserRound size={14} />
                )
              }
              accentClass={template.bannerAccent}
    ...
    const renderProfilePhoto = (
      ...
      const altText = data.personal.name.trim()
        ...
        : '简历照片'
      const baseSize =
        variant === 'centered' ? 80 : variant === 'banner' ? 76 : 74
            ...
            <header {...getPersonalSectionProps(dividerClass)}>
              <div
                className={`flex gap-6 ${
                  hasHeaderColumns ? 'items-start justify-between' : 'flex-col'
                }`}
              >
                ...
                <div className='min-w-0 flex-1'>
                  ...
                  {data.personal.title.trim() && (
                    <p
                      className={`text-lg ${personalTitleToneClass} font-medium mt-1`}
                    >
                      {data.personal.title}
                    </p>
                  )}
              ...
              <header {...getPersonalSectionProps(dividerClass)}>
                ...
                >
                  ...
                  <div className='min-w-0 flex-1 text-center'>
                    ...
                    {data.personal.title.trim() && (
                      <p
                        className={`text-base ${personalTitleToneClass} font-medium mt-1`}
                      >
                        {data.personal.title}
                      </p>
                    )}
              ...
              <div className={`border-l-4 ${c.primaryBorder} pl-4`}>
                <div className='flex items-start justify-between gap-6'>
                  ...
                  <div className='min-w-0 flex-1'>
                    ...
                    {data.personal.title.trim() && (
                      <p
                        className={`text-base ${personalTitleToneClass} font-semibold mt-1`}
                      >
                        {data.personal.title}
                      </p>
                    )}
    ...
    const renderExperienceHeader = (exp: Experience, compact = false) => {
      const role = sectionPreferences.experience.showRole ? exp.role.trim() : ''
      const roleInMiddle =
          ...
          <div className='print-item-header mb-1'>
            ...
            >
              <div className='flex min-w-0 flex-wrap items-baseline gap-x-2 gap-y-0.5'>
                <h3
                  className={`resume-item-title min-w-0 font-bold text-base ${c.heading}`}
                >
                  {exp.company}
                </h3>
                {roleOnTitle && (
                  <span
                    className={`text-sm font-medium ${experienceRoleToneClass}`}
                  >
                    {role}
                  </span>
                )}
            ...
            {roleAtBottom && (
              <div
                className={`mt-1 text-sm font-medium ${experienceRoleToneClass}`}
              >
                {role}
              </div>
            )}
        ...
        <div className='print-item-header'>
          ...
          {(roleAtBottom || dateBelow) && (
            <div className='mb-2 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-sm'>
              {roleAtBottom && (
                <span className={`font-medium ${experienceRoleToneClass}`}>
                  {role}
                </span>
              )}
              {roleAtBottom && dateBelow && (
    ...
    const renderProjectTag = (tags: string) => {
      ...
      return (
        <span className={`rounded border px-2 py-0.5 text-xs ${tagClassName}`}>
          {text}
        </span>
      )
    }
    ...
    const renderProject = (proj: Project) => {
      ...
      const date = sectionPreferences.projects.showDates ? proj.date.trim() : ''
      const role = sectionPreferences.projects.showRole ? proj.role.trim() : ''
      const roleInMiddle =
          ...
          <div className='print-item-header mb-1'>
            ...
            >
              <div className='flex min-w-0 flex-wrap items-baseline gap-x-3 gap-y-1'>
                <h3
                  className={`resume-item-title font-bold text-base ${c.heading}`}
                >
                  {proj.name}
                </h3>
                {roleOnTitle && (
                  <span
                    className={`text-sm font-medium ${projectRoleToneClass}`}
                  >
                    {role}
                  </span>
                )}
